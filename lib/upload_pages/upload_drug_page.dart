...
class UploadDrugScreen extends ConsumerStatefulWidget {
  const UploadDrugScreen({super.key});
  // this.selectedImages});
  // final List<AssetEntity>? selectedImages;
  @override
  ConsumerState<UploadDrugScreen> createState() => _UploadDrugScreenState();
}

class _UploadDrugScreenState extends ConsumerState<UploadDrugScreen> {
  ...
  void _openImagePicker() async {
    final List<AssetEntity>? images = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => const MultipleImagePicker(),
      ),
    );
    ...
    if (images != null && images.isNotEmpty) {
      setState(() {
        ...
        _imageFutures = images.map((image) => image.originBytes).toList();
      });
  ...
  void handleItemUpload(BuildContext context) async {
    ...
    if (selectedStore != null) {
      ...
      if (selectedImages.isNotEmpty) {
        for (AssetEntity imageEntity in selectedImages) {
  ...
  // void _loadImages() {
  //   _imageFutures = selectedImages.map((image) => image.originBytes).toList();
  // }
                  ...
                  child: Column(
                    children: [
                      ...
                      categoriesAsyncValue.when(
                        data: (types) {
                          ...
                          if (matchedCategory == null) {
                            return const Text(
                                'No category found for this store');
                          }
                      ...
                      const Row(
                        mainAxisAlignment: MainAxisAlignment.start,
                        children: [
                          Text('Product Dosage / Prescription ',
                              style: TextStyle(fontWeight: FontWeight.w700)),
                          Text('*',
                              style: TextStyle(color: AppColors.primaryColor)),
                        ],
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          ...
                          fillColor: Colors.grey[300],
                          hintText:
                              'Briefly describe dosage/prescription here ...',
                          border: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(6.0),
                            borderSide: BorderSide.none,
                          ),
